 ...
 * all arguments are utilized and it covers the most practical cases that were originally
 * given along with an additional one to show it is not a biased solution.
 * 
 ...
 */
function seatReservations(N: number, K: number, seat: number[]) {
  let reservations: number[] = new Array();
  let freeSeats: number[] = new Array();
  let nextFreeSeat: number = 1;

  for (let i = 0; i < K; i++) {
    let operation: number = seat[i];

    if (operation === 0) {
      if (freeSeats.length !== 0) {
        let lowestAvailableSeat: number = Math.min(...freeSeats);
        reservations.push(lowestAvailableSeat);

        let index: number = freeSeats.indexOf(lowestAvailableSeat);
...
let testArray1 = [0, 0, 0, 2, 1, 0];
let testArray2 = [0, 0, 0, 2, 0];
let testArray3 = [0, 0, 0, 2, 0, 0, 1, 0];
let testArray4 = new Array(10).fill(0);
let testArray5 = [0,0,0,3,2,0,0];
